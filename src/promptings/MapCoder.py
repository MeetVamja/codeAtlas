...
class MapCoder(BaseStrategy):
    ...
    @classmethod
    def _extract_tag(cls, tag: str, text: str) -> List[str]:
        """Best-effort tag content extraction that tolerates malformed/unclosed
        XML and CDATA sections from weaker LLMs (unlike a strict ET parser)."""
        pattern = rf'<{tag}>(.*?)</{tag}>'
        matches = re.findall(pattern, text, re.DOTALL)
        return [cls._strip_cdata(m) for m in matches]
    ...
    def parse_xml(self, response: str) -> dict:
        ...
        problem_blocks = self._extract_tag('problem', response)
        if problem_blocks:
            result['problem'] = []
            for block in problem_blocks:
                description = self._extract_tag('description', block)
                code = self._extract_tag('code', block)
                planning = self._extract_tag('planning', block)
                result['problem'].append({
                    'description': description[0] if description else '',
                    'code': code[0] if code else '',
                    'planning': planning[0] if planning else '',
                })

        for tag in ('algorithm', 'explanation', 'confidence'):
            values = self._extract_tag(tag, response)
            if values:
                result[tag] = values[-1]
...
""",
        ...
        for example_no, example in enumerate(response["problem"], start=1):
            ...
            confidence_match = re.search(
                r'-?\d+', str(verification_res.get('confidence', '')))
            verification_res['confidence'] = int(
                confidence_match.group()) if confidence_match else 50
